...
public final class PropertyResolver
{
	...
	private final static ConcurrentHashMap<Object, ClassCache> applicationToClassesToGetAndSetters = new ConcurrentHashMap<>(2);
	 ...
	 * then a RuntimeException will be thrown. If a null object is encountered then it will
		...
		final Object value)
	{
		...
		{
			throw new RuntimeException("Empty expression setting value: " + value +
				" on object: " + object);
		}
		if (object == null)
		{
	 ...
	 * @throws RuntimeException if the cannot be resolved
	 */
	 ...
	 * @throws RuntimeException if class cannot be resolved
	 */
	 ...
	 * @throws RuntimeException if there is no such field
	 */
	 ...
	 * @throws RuntimeException if there is no getter method
	 */
	 ...
	 * @throws RuntimeException if there is no setter method
	 */
		...
		final Object object, final int tryToCreateNull, Class<?> clz)
	{
		...
		GetAndSet getAndSetter = getGetAndSetter(exp, clz);
	...
	{
		Object key = PropertyResolver.class;

		ClassCache result = applicationToClassesToGetAndSetters.get(key);
		if (result == null)
		{
			ClassCache tmpResult = applicationToClassesToGetAndSetters.putIfAbsent(key, result = new DefaultClassCache());
			if (tmpResult != null)
			{
				result = tmpResult;
			}
